...
export function CreatePage() {
  ...
  async function create() { try { setError(""); const next = await api.post<Job>("/jobs", { workflow: mode, preset, watermark }); const send = async (kind: string, file: File) => { const form = new FormData(); form.append("file", file); const response = await fetch(`/api/v1/jobs/${next.id}/inputs/${kind}`, { method: "POST", body: form }); if (!response.ok) throw new Error(await response.text()); }; await send("portrait", portrait!); await send(mode === "speech" ? "audio" : "reference", mode === "speech" ? audio! : reference!); setJob(await api.post<Job>(`/jobs/${next.id}/start`)); } catch { setError("AvatarKit could not validate or queue these local files. Check Diagnostics and media guidance."); } }
