...
export type Job = { id: string; state: string; phase: string; workflow: string; preset: string; created_at: string; output_path?: string; error_message?: string };
export const api = {
  ...
  delete: (path: string) => fetch(`${root}${path}`, { method: "DELETE" }).then(async r => { if (!r.ok) throw new Error(await r.text()); }),
};
