$ErrorActionPreference = 'Stop'; $root = (Resolve-Path "$PSScriptRoot\..\..").Path; if (-not $env:AVATARKIT_HOME) { $env:AVATARKIT_HOME = Join-Path $env:USERPROFILE '.avatarkit' }; $python = Join-Path $env:AVATARKIT_HOME 'environments\backend\Scripts\python.exe'
